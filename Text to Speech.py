...
#to delete saved audio files when tkinter-app closes (EXCEPT FILES SAVED WITH 'SAVE AS MP3' BUTTON)
import os
...
def choose_file(event):
    try:
        ...
        entry_displayFilePath.delete(0, END) #removing the line "No file chosen"
        entry_displayFilePath.insert(END, file_name) #and writing the path of chosen text-file
        ...
        entry_displayFilename.delete(0, END) #removing the line "No file chosen"
        entry_displayFilename.insert(END, file_name[file_name_ind:]) #and writing the name of chosen text-file

        text_displayFileContent.delete("1.0", "end") #removing the line "No file chosen"
        text_displayFileContent.insert(END, file_content[0]) #and writing the content of chosen text-file
...
lable_enterText = Label(w, text="Enter the text:", bg='#fff', font=('segoeui',14))
lable_displayOR = Label(w, text="OR", bg='#fff', font=('segoeui',14))
...
lable_enterText.pack()
lable_displayOR.pack()
...
lable_enterText.place(x = 50, y = 100)
lable_displayOR.place(x = 100, y = 140)
